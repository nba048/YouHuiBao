...
/**
 * 1. 创建BaseActivity继承ActionbarActivity(导入v7包) 2.
 * 创建状态选择器bg_tab_text和tab_text_color 3. 自动导包 4. 给UIUtils增加方法:getColorStateList
 *
 * @author Kevin
 * @date 2015-10-27
 */
public class PagerTab extends ViewGroup {
    ...
    private int mDividerColor = 0xaa53cac3;// 分割线颜色
    // private int mDividerColor = 0xffe2e5e5;// 分割线颜色
    ...
    private int mIndicatorColor = 0xaa53cac3;// 指示器颜色
    // private int mIndicatorColor = 0xffe2e5e5;// 指示器颜色
    ...
    private int mTabBackgroundResId = R.drawable.wheel_bg;// tab背景资源
    private int mTabTextColorResId = R.color.bg_tab_selector_text; // tab文字颜色
    ...
    private void init() {
        // 把一个值从dip转换成px
        mIndicatorHeight = ProjectUtil.dip2px(mIndicatorHeight);
        mDividerPadding = ProjectUtil.dip2px(mDividerPadding);
        mTabPadding = ProjectUtil.dip2px(mTabPadding);
        mDividerWidth = ProjectUtil.dip2px(mDividerWidth);
        mTabTextSize = ProjectUtil.dip2px(mTabTextSize);
        // 创建一个scroller
    ...
    public void addTextTab(final int position, String title) {
        ...
        tab.setTextColor(ProjectUtil.getColorStateList(mTabTextColorResId));
        tab.setBackgroundDrawable(ProjectUtil.getDrawable(mTabBackgroundResId));
