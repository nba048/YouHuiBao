...
import com.yhb.mvp.view.adapter.TradeTotalAdapter;
 ...
 * @author 宋昌明
 */
public class TradeTotalListActivity extends BaseActivity implements OnClickListener, ITradeTotalListView<TradeSituation>, OnDateSetListener, AbsListView.OnScrollListener {
    ...
    private ITradeTotalListPre mPre;
    private List<TradeSituation> mDatas;
    private List<TradeSituation> mDatasSearch;
    private TradeTotalAdapter mAdapter;
    ...
    protected void onLCreate() {
        try {
            ...
            mPre = ProjectFactroy.newInstanceByTradeTotalListPre(this);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    protected void initView() {
        ...
        View headView = LayoutInflater.from(this).inflate(R.layout.order_total_heard_layout, null, false);
...
//        AbsListView.LayoutParams localObject = new AbsListView.LayoutParams(mScreenWidth, (int) (4.0F * (mScreenWidth / 16.0F)));
    ...
    protected void initData() {
        ...
        mPre.lodingDatas(mCurrentFunction.getUrl(), tvTimeStart.getText().toString(), tvTimeStart.getText().toString());
        mDialog.show();
    ...
    public void onClick(View v) {
        if (v == ibFoot) {
            ...
            mDialog.show();
            mPre.lodingMoreDatas(mCurrentFunction.getUrl(), tvTimeStart.getText().toString(), tvTimeStart.getText().toString());
        }
    }
    ...
    @Override
    public void setDatas(List<TradeSituation> datas) {
        if (mDatas == null) {
            mDatas = datas;
        } else {
            mDatas.clear();
    ...
    private void initEtNum() {
        if (etMinNum.getText().toString().length() > 0 && etMaxNum.getText().toString().length() > 0) {
            ...
            ArrayList<TradeSituation> list = new ArrayList<>();
            list.addAll(mDatas);
            for (TradeSituation item : list) {
                try {
                    int current = Integer.parseInt(item.getTotalMoney());
                    if (current < min) {
                        mDatas.remove(item);
                    } else if (current > max) {
                        mDatas.remove(item);
    ...
    private void initList() {
        ...
        if (mAutoAdapter == null) {
            ...
            for (TradeSituation item : mDatas) {
    ...
    public void showList() {
        if (mAdapter == null && mDatas != null) {
            mAdapter = new TradeTotalAdapter(this, R.layout.item_trade_total_layout, mDatasSearch);
    ...
    @Override
    public void showMoreData(List<TradeSituation> datas) {
        if (mDatas != null) {
            mDatas.addAll(datas);
            initList();
            mDialog.dismiss();
