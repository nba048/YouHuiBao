...
import com.yhb.mvp.view.adapter.OrderTotalAdapter;
 ...
 * @author 宋昌明
 */
public class OrderTotalListActivity extends BaseActivity implements OnClickListener, IOrderTotalListView<OrderSituation>, OnDateSetListener, AbsListView.OnScrollListener {
    ...
    private IOrderTotalListPre mPre;
    private List<OrderSituation> mDatas;
    private List<OrderSituation> mDatasSearch;
    ...
    protected void onLCreate() {
        try {
            ...
            mPre = ProjectFactroy.newInstanceByOrderTotalListPre(this);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
...
//        AbsListView.LayoutParams localObject = new AbsListView.LayoutParams(mScreenWidth, (int) (4.0F * (mScreenWidth / 16.0F)));
    ...
    @Override
    public void setDatas(List<OrderSituation> datas) {
        if (mDatas == null) {
            mDatas = datas;
        } else {
            mDatas.clear();
    ...
    private void initEtNum() {
        if (etMinNum.getText().toString().length() > 0 && etMaxNum.getText().toString().length() > 0) {
            ...
            ArrayList<OrderSituation> list = new ArrayList<>();
            list.addAll(mDatas);
            for (OrderSituation item : list) {
                try {
    ...
    private void initList() {
        ...
        if (mAutoAdapter == null) {
            ...
            for (OrderSituation item : mDatas) {
    ...
    public void showList() {
        if (mAdapter == null && mDatas != null) {
            mAdapter = new OrderTotalAdapter(this, R.layout.item_order_total_layout, mDatasSearch);
    ...
    @Override
    public void showMoreData(List<OrderSituation> datas) {
        if (mDatas != null) {
            mDatas.addAll(datas);
            initList();
            mDialog.dismiss();
