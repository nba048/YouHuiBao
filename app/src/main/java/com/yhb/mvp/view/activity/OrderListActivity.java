 ...
 * @author 宋昌明
 */
public class OrderListActivity extends BaseActivity implements OnClickListener, IOrderListView<Order>, OnDateSetListener, AbsListView.OnScrollListener {
    ...
    private List<Order> mDatasSearch;
...
//        AbsListView.LayoutParams localObject = new AbsListView.LayoutParams(mScreenWidth, (int) (4.0F * (mScreenWidth / 16.0F)));
    ...
    private void initList() {
        ...
        if (mAutoAdapter == null) {
            ...
            for (Order item : mDatas) {
                ...
                mAutoDataList.add(item.getFirmName());
                mAutoDataList.add(item.getFirmIp());
                mAutoDataList.add(item.getOrderNo());
            }
            resetList();
    ...
    public void showList() {
        if (mAdapter == null && mDatas != null) {
            mAdapter = new OrderAdapter(this, R.layout.item_order_layout, mDatasSearch);
