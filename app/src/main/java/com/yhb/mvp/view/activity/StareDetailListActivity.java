...
import com.yhb.mvp.view.adapter.StareDetailAdapter;
 ...
 * @author 宋昌明
 */
public class StareDetailListActivity extends BaseActivity implements OnClickListener, IStareDetailListView<FrequentTrading>, OnDateSetListener, AbsListView.OnScrollListener {
    ...
    private Frequent mCurrentFrequent;
    ...
    private IStareDetailListPre mPre;
    private List<FrequentTrading> mDatas;
    private List<FrequentTrading> mDatasSearch;
    private StareDetailAdapter mAdapter;
    ...
    protected void onLCreate() {
        try {
            mCurrentFrequent = (Frequent) getIntent().getSerializableExtra("mFrequents");
            mMarketName = getIntent().getStringExtra("mMarketName");
            mPre = ProjectFactroy.newInstanceByStareDetailListPre(this);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
...
//        AbsListView.LayoutParams localObject = new AbsListView.LayoutParams(mScreenWidth, (int) (4.0F * (mScreenWidth / 16.0F)));
    ...
    protected void initData() {
        ...
        tvModelName.setText("交易商ID： " + mCurrentFrequent.getFirmId());
        mPre.lodingDatas(mCurrentFrequent.getId());
    ...
    public void onClick(View v) {
        if (v == ibFoot) {
            ...
            mDialog.show();
            mPre.lodingMoreDatas(mCurrentFrequent.getId());
        }
    }
    ...
    @Override
    public void setDatas(List<FrequentTrading> datas) {
        if (mDatas == null) {
            mDatas = datas;
        } else {
            mDatas.clear();
    ...
    private void initList() {
        ...
        if (mAutoAdapter == null) {
            ...
            for (FrequentTrading item : mDatas) {
        ...
        if (mSpPosition2 != 0) {
            if (mSpPosition2 == 1) {
                Collections.sort(mDatasSearch, new Comparator<FrequentTrading>() {
                    @Override
                    public int compare(FrequentTrading lhs, FrequentTrading rhs) {
    ...
    public void showList() {
        if (mAdapter == null && mDatas != null) {
            mAdapter = new StareDetailAdapter(this, R.layout.item_stare_detail_layout, mDatasSearch);
    ...
    @Override
    public void showMoreData(List<FrequentTrading> datas) {
        if (mDatas != null) {
            mDatas.addAll(datas);
            initList();
            mDialog.dismiss();
